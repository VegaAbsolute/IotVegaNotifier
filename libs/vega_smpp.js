...
class VegaSMPP extends EventEmitter
{
    ...
    if(this._connect._status)
    {
      for(let i = 0; i < this._stack.length; i++)
      {
        ...
        if(!item.status)
        {
          ...
          .then((res)=>{
           if(res.status)
           {
             ...
             for(let j = 0 ; j < _self._stack.length; j++)
             {
               ...
               {
                 console.log(moment().format('LLL')+':  [WS]'+'Success to send sms message '+_self._stack[j].telephone);
           ...
           else
           {
             for(var j = 0 ; j < _self._stack.length;j++)
             {
               if(_self._stack[j].uuid === res.uuid)
               {
                 ...
                 _self.checkStackEmptiness();
                 console.log(moment().format('LLL')+':  [WS]'+'failed to send  sms message '+tmp.telephone);
               }
             }
         ...
         .catch((e)=>{
           item.status = false;
           console.log(moment().format('LLL')+':  [WS]'+'failed to send  sms message. Error 1');
  ...
  _connectSMPP()
  {
    ...
    this.bind_transceiver(this._system, function(pdu) {
      ...
      {
          _self._status = true;
          console.log(moment().format('LLL')+':  [WS]'+'Successful connection on SMPP ');
      }
      else if(pdu.command_status == 5) {
        _self._status = false;
        _self.unbind();
        console.log(moment().format('LLL')+':  [WS]'+'Not successful connection on SMPP, status = 5');
      }
      else
      {
        _self._status = false;
        console.log(moment().format('LLL')+':  [WS]'+'Not successful connection on SMPP, status ='+pdu.command_status);
      }
    });
  ...
  _pdu(pdu)
  {
    if (pdu.command == 'deliver_sm')
    {
      try
      {
        ...
        console.log(moment().format('LLL')+':  [WS]'+'SMS ' + fromNumber + ' -> ' + toNumber + ': ' + text);
        // Reply to SMSC that we received and processed the SMS
        this.deliver_sm_resp({ sequence_number: pdu.sequence_number });
      }
      catch(err)
      {
        console.log(moment().format('LLL')+':  [WS]'+'Error event pdu deliver_sm, ',err);
        console.dir(moment().format('LLL')+':  [WS]'+'pdu',pdu);
      }
    }
  }
  _close()
  {
    console.log(moment().format('LLL')+':  [WS]'+'smpp disconnected');
    this._status = false;
  }
  _error(error)
  {
    console.log(moment().format('LLL')+':  [WS]'+'smpp error', error);
    this._status = false;
  }
    ...
    return new Promise((resolve, reject)=>{
      try
      {
        ...
        _self._connect.submit_sm(mess, function(pdu)
        {
            ...
            {
              console.log(moment().format('LLL')+':  [WS]'+pdu.command_status);
